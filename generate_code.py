...
def RunGenerators(registry: str, output_directory: str):

    # These live in the Vulkan-Docs repo, but are pulled in via the
    # Vulkan-Headers/registry folder
    # At runtime we inject python path to find these helper scripts
    registry_headers_path = os.path.join(registry)
    print(registry_headers_path)
    ...
    vk_xml = os.path.abspath(os.path.join(registry, 'vk.xml'))
...
if __name__ == '__main__':
    ...
    registry = os.path.abspath(os.path.join(os.path.dirname(__file__), 'Vulkan-Headers', 'registry'))
    ...
    RunGenerators(registry, output_directory)
